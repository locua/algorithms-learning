...
void HeapConstructorsTest::testIncrementalConstructor() {
 std::vector<int> a = {82,10,75,40,46,39,42,57};
 std::vector<int> b = {67,88,30,1,55,97,36,76};
  std::vector<int> v = {17};
  Heap h = Heap(v, true);
  ...
  h = Heap(z, true); //  CPPUNIT_ASSERT_EQUAL_MESSAGE("The heap from figure 4.16 of Ida's subject guide should have length 8", 8U, h.heap_size);
...
void HeapConstructorsTest::counter() {
    // std::vector<int> v = {};
    // for (int i = 0; i < 59985; i++) {
    //     v.push_back(i);
    // }

    // Heap h = Heap(v, true);
    // //Heap h = Heap(v, false);
    // std::cout << "Count: " << h.counter.report() << std::endl;
}
