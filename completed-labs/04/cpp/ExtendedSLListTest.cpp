...
// void ExtendedSLListTest::testLength() {
//   CPPUNIT_ASSERT_EQUAL_MESSAGE("Not getting the right rest (index 0) from length()", n, n->length());
//   // CPPUNIT_ASSERT_EQUAL_MESSAGE("Not getting the right rest (index 1) next from length()", n->length(), n->length(1));
//   // CPPUNIT_ASSERT_EQUAL_MESSAGE("Not getting the right rest (index 2) next from length()", n->()->rest(), n->nthRest(2));
//   // CPPUNIT_ASSERT_EQUAL_MESSAGE("Not getting the right rest (index 3) next from length()", SLList::NIL, n->length(3));
//   for (int k = 0; k < 100; k++) {
//     int i = rand() % 1000;
//     CPPUNIT_ASSERT_EQUAL(ncirc->rest(), ncirc->length());
//   }
// }
